...
server <- function(input, output) {
    ...
    output$song_name <- renderUI({
      ...
      get_song_name(df, input$school_name)
    ...
    p <- ggplot(data = songs_df, aes(x = sec_duration, y = bpm, text = input$school_name)) + geom_point(aes(col = input$school_name)) + 
      labs(y = "Beats per Minute", x = "Song Duration")
